<script setup>
    ...
    const openEditItem = (employerId) => {
        // 1. Find the item in the local list
        const employerToEdit = props.employers.find(c => c.id === employerId);

        // 2. Set the state variable. This triggers the watcher in the modal.
        currentItem.value = employerToEdit;
    ...
    const addRouteUrl = computed(() => {
        return route('employers.store').toString();
    });

    const updateRouteName = computed(() => 'employers.update');
</script>
    ...
    <div class="px-4 sm:px-6 lg:px-8 max-w-xl">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">A list of all your employers.</p>
            </div>
                        ...
                        <tr v-for="employer in employers" :key="employer.id"
                            ...
                            <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-3 dark:text-white">
                             ...
                             <td class="p-4 text-sm text-red-500">
                                <button @click="openEditItem(employer.id)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
        ...
        >
            <p class="mt-4 text-gray-700 font-semibold">
                ...
                <input
                    ...
                    placeholder="The name of the employer"
                />
            </p>
